// AUTO-GENERATED by scripts/build-tokens.mjs — DO NOT EDIT (SSOT: tokens/*.json)
export type StylePreset = 'bootstrap' | 'tailwind' | 'toss'
export type ColorToken = 'primary' | 'secondary' | 'error' | 'success' | 'warning' | 'neutral' | 'bg' | 'bgSubtle' | 'text' | 'border'
/** 팔레트 톤(셰이드·solid·on-color가 파생되는 base 키) */
export type PaletteToken = 'primary' | 'secondary' | 'error' | 'success' | 'warning' | 'neutral'
/** solid 면 — 흰 글자가 AA를 통과하는 톤 면(--ds-color-solid-*). base에서 계산된 파생 토큰. */
export type SolidColorToken = 'solid-primary' | 'solid-secondary' | 'solid-error' | 'solid-success' | 'solid-warning' | 'solid-neutral'
/** solid 면의 전경색 — base에서 계산된 파생 토큰(--ds-color-on-*). WCAG AA 4.5:1 보장. */
export type OnColorToken = 'on-primary' | 'on-secondary' | 'on-error' | 'on-success' | 'on-warning' | 'on-neutral'
export type FontSizeToken = 'xs' | 'sm' | 'md' | 'lg' | 'xl' | 'xxl'
export type FontWeightToken = 'regular' | 'medium' | 'bold'
export type RadiusToken = 'sm' | 'md' | 'lg'
export type SpacingToken = '1' | '2' | '3' | '4' | '5' | '6'

export interface DesignTokens {
  $preset: StylePreset
  color: Record<ColorToken, string>
  typography: {
    fontFamily: string
    baseSize: number
    scale: number
    sizes: Record<FontSizeToken, number>
    weights: Record<FontWeightToken, number>
  }
  radius: Record<RadiusToken, number>
  spacing: Record<SpacingToken, number>
}

export const presets: Record<StylePreset, DesignTokens> = {
  "bootstrap": {
    "$preset": "bootstrap",
    "color": {
      "primary": "#7952B3",
      "secondary": "#6C757D",
      "error": "#DC3545",
      "success": "#198754",
      "warning": "#FFC107",
      "neutral": "#212529",
      "bg": "#FFFFFF",
      "bgSubtle": "#F8F9FA",
      "text": "#212529",
      "border": "#DEE2E6"
    },
    "typography": {
      "fontFamily": "system-ui, -apple-system, 'Segoe UI', Roboto, sans-serif",
      "baseSize": 16,
      "scale": 1.25,
      "sizes": {
        "xs": 10,
        "sm": 13,
        "md": 16,
        "lg": 20,
        "xl": 25,
        "xxl": 31
      },
      "weights": {
        "regular": 400,
        "medium": 500,
        "bold": 700
      }
    },
    "radius": {
      "sm": 4,
      "md": 8,
      "lg": 12
    },
    "spacing": {
      "1": 4,
      "2": 8,
      "3": 12,
      "4": 16,
      "5": 20,
      "6": 24
    }
  },
  "tailwind": {
    "$preset": "tailwind",
    "color": {
      "primary": "#06B6D4",
      "secondary": "#64748B",
      "error": "#EF4444",
      "success": "#22C55E",
      "warning": "#F59E0B",
      "neutral": "#0F172A",
      "bg": "#FFFFFF",
      "bgSubtle": "#F9FAFB",
      "text": "#0F172A",
      "border": "#E5E7EB"
    },
    "typography": {
      "fontFamily": "'Inter', system-ui, sans-serif",
      "baseSize": 16,
      "scale": 1.25,
      "sizes": {
        "xs": 10,
        "sm": 13,
        "md": 16,
        "lg": 20,
        "xl": 25,
        "xxl": 31
      },
      "weights": {
        "regular": 400,
        "medium": 500,
        "bold": 700
      }
    },
    "radius": {
      "sm": 4,
      "md": 8,
      "lg": 12
    },
    "spacing": {
      "1": 4,
      "2": 8,
      "3": 12,
      "4": 16,
      "5": 20,
      "6": 24
    }
  },
  "toss": {
    "$preset": "toss",
    "color": {
      "primary": "#3D6BFF",
      "secondary": "#4E5968",
      "error": "#F04452",
      "success": "#00C471",
      "warning": "#FF9F0A",
      "neutral": "#191F28",
      "bg": "#FFFFFF",
      "bgSubtle": "#F5F7FA",
      "text": "#191F28",
      "border": "#E5E8EB"
    },
    "typography": {
      "fontFamily": "'Pretendard', -apple-system, sans-serif",
      "baseSize": 16,
      "scale": 1.2,
      "sizes": {
        "xs": 11,
        "sm": 13,
        "md": 16,
        "lg": 19,
        "xl": 23,
        "xxl": 28
      },
      "weights": {
        "regular": 400,
        "medium": 500,
        "bold": 700
      }
    },
    "radius": {
      "sm": 4,
      "md": 8,
      "lg": 12
    },
    "spacing": {
      "1": 4,
      "2": 8,
      "3": 12,
      "4": 16,
      "5": 20,
      "6": 24
    }
  }
}
