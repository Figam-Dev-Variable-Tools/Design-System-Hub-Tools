...
/**
 * Labels: 영문 오버라이드 — 상태 맵·탭·섹션 제목·표 머리글 3벌·사이드 카드·확인창까지
 * labels 통로 하나로 전부 영문이 된다(수량 단위는 labels.units, 자릿수·통화는 formatters의 몫).
 */
export const Labels: Story = {
  args: {
    labels: {
      status: { onSale: 'On sale', soldOut: 'Sold out', stopped: 'Stopped', draft: 'Draft' },
      inquiryStatus: { waiting: 'Waiting', answered: 'Answered', closed: 'Closed' },
      flow: { draft: 'Created', review: 'Review', onSale: 'On sale', stopped: 'Ended' },
      tabs: { sales: 'Sales', orders: 'Recent orders', inquiries: 'Inquiries' },
      sections: {
        gallery: 'Images',
        basic: 'Basics',
        options: 'Options',
        ...
        timeline: 'Sales progress',
        manager: 'Owners',
        quickActions: 'Quick actions',
      },
      meta: { code: 'SKU', createdAt: 'Created', updatedAt: 'Updated', createdBy: 'Created by' },
      basic: {
        price: 'Price',
        stock: 'Stock',
        ...
        taxed: 'Taxable',
        taxFree: 'Tax free',
        belowSafety: 'below safety stock',
      ...
      stats: {
        ...
        soldOutOptions: 'Sold-out options',
        sold6m: 'Sold (6mo)',
        openInquiries: 'Open inquiries',
        safetyHint: (stock) => `Safety stock ${stock}`,
        byOption: 'by option',
        noOption: 'no options',
      ...
      gallery: {
        zoom: 'Zoom',
        coverBadge: 'Cover',
        countHint: (count) => `${count} images · the first one is the cover`,
        zoomAria: ({ name, index }) => `Zoom image ${index + 1} of ${name}`,
      },
      visibility: { display: 'Storefront', displayHint: 'Listed and searchable', sale: 'Selling' },
      taxonomy: { edit: 'Edit', category: 'Category', tags: 'Tags', emptyTags: 'No tags yet' },
      description: { attachments: 'Files' },
      chart: { title: 'Units sold, last 6 months', series: 'Units' },
      ...
      empty: {
        ...
        descriptionBody: 'No description yet',
        sales: 'No sales data yet',
      ...
      duplicateDialog: {
        ...
        description: (p) => `A draft copy of ${p.name} will be created.`,
        confirmLabel: 'Duplicate',
      },
      deleteDialog: {
        title: 'Delete this product?',
        description: (p) => `${p.name} (${p.code}) will be removed.`,
      },
      units: { count: ' pcs' },
    },
    formatters: {
